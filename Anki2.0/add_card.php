...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $filteredDeckName = $_GET["data"];

    $sql = "SELECT id FROM deck WHERE deckName = '$filteredDeckName'";

    $result = $conn->query($sql);
    $row = $result->fetch_assoc();

    if (!empty($front_side) && !empty($back_side)) {
        $sql = "INSERT INTO deckDetails (deck_id, front_side, back_side) VALUES (" . $row["id"] . ", '$front_side', '$back_side')";
